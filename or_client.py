...
# --- NVIDIA Provider (additional text model source, tried before the
#     OpenRouter pool). Uses NVIDIA's OpenAI-compatible /v1/chat/completions
#     endpoint. Fill in a real key to activate it; while the placeholder is
#     left in place, calls to this provider fail fast and the client falls
#     through to OpenRouter automatically. ---
NVIDIA_API_KEY  = "YOUR_NVIDIA_API_KEY"  # Placeholder — replace with your NVIDIA NIM API key
...
class OpenRouterClient:
    ...
    def __init__(self) -> None:
        ...
        self._nvidia_headers = {
            "Authorization": f"Bearer {NVIDIA_API_KEY}",
            "Content-Type":  "application/json",
        }

    def _provider_config(self, provider: str) -> tuple[str, dict]:
        """Return (api_url, headers) for the given provider name."""
        if provider == "nvidia":
            return NVIDIA_API_URL, self._nvidia_headers
        return API_URL, self._headers
    ...
    def _call(
        ...
        api_url, headers = self._provider_config(provider)
        tag = "NVIDIA" if provider == "nvidia" else "OpenRouter"

        if provider == "nvidia":
            if not NVIDIA_API_KEY or NVIDIA_API_KEY == "YOUR_NVIDIA_API_KEY":
                # Placeholder key still in place — skip straight to fallback pool.
                return None
        ...
        for attempt in range(1, MAX_RETRIES_PER_MODEL + 1):
            try:
                ...
                if resp.status_code == 401:
                    if provider == "nvidia":
                        logger.warning(f"[NVIDIA] Authentication failed for model {model}. Falling back.")
                        return None
                    raise PermissionError(
                ...
                if resp.status_code == 403:
                    if provider == "nvidia":
                        logger.warning(f"[NVIDIA] Access denied for model {model} (HTTP 403). Falling back.")
                        return None
                    raise PermissionError(
                ...
                logger.warning(
                    f"[{tag}] {model} → HTTP {resp.status_code} "
                ...
                logger.error(f"[{tag}] {model} → Unexpected error: {e}")
    ...
    def _call_with_fallback(
        ...
        temperature: float = DEFAULT_TEMPERATURE,
        response_format: Optional[dict] = None,
        nvidia_pool: Optional[list[str]] = None,
    ) -> str:
        ...
        # Try NVIDIA provider first (e.g. meta/llama-3.3-70b-instruct), when available.
        for m in (nvidia_pool or []):
            logger.info(f"[NVIDIA] Trying: {m}")
            result = self._call(m, messages, max_tokens, temperature, response_format, provider="nvidia")
            if result:
                logger.info(f"[NVIDIA] ✓ Success: {m}")
                return result
    ...
    def chat(
        ...
        prompt: str,
        system: str = (
            "You are a component of Brahma AI - Lite, an open-source personal assistant. "
        ...
        return self._call_with_fallback(
            ...
            nvidia_pool=NVIDIA_TEXT_MODELS,
        )
...
if __name__ == "__main__":
    print("=" * 55)
    print("  Brahma AI - Lite — OpenRouter Client Self-Test")
    print("=" * 55)
